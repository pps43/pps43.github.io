...
在提供的matlab仿真程序中，对参数 SampleDensity（信道生成使用的采样密度，和多普勒频移参数有关）和Ts（用于将delay从具体时间转化到抽头数，也是通信系统的采样率）说明的不完善，需要自己玩味。另外，最终生成的信道存储在$\mathbf{H}$中，是一个五维的矩阵，每一维的具体含义文档里说的很清楚了： ${\mathbf{H}}(i,j,k,p,q)$表示第$q$条链路在第$p$个采样时间点上，从天线$i$到天线$j$之间delay为$k$秒的信道增益。