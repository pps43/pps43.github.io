...
[上一篇]({{< ref "/posts/asking_ai_about_ai/1">}})我们聊了AI模型的分类和大致架构，目前最火的两类模型`LLM`和`Diffusion`都基于`Transformer`架构训练而成，`ChatGPT`则是`LLM`家族中最出色的产品。本文就来聊聊最近出现的基于`ChatGPT`的创新架构和应用。由于`ChatGPT`的训练数据截止到2021年9月，且笔者并没有Plus会员才能使用的`ChatGPT Plugin`，所以这次请到的是[Dr. Phind](https://www.phind.com/)（The AI search engine for developers），大家欢迎。
...
>   ![The Generative Agent Architecture](GenerativeAgentArchitecture.webp)
>
...
>   ![BabyAGI_Architecture](BabyAGI_Architecture.webp)
> 
