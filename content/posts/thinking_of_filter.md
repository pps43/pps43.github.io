...
$$\hat S = {\mathbf{T}}\left( X \right)$$

${\mathbf{T}}\left( \bullet \right)$ 表示一种变换，${\mathbf{E}}\left( \bullet \right)$ 表示求期望。根据LMS算法的结果，对发送信号的估计值应为：

$$\hat S = {\mathbf{E}}(S|X)$$

这显然是难以获知的后验信息，所以这个结果仅仅有理论指导意义。

如果多加一层约束，规定：${\mathbf{T}}\left( \bullet \right)$ 为线性运算，则运用信号与系统的观点，可以将这个线性系统用冲激响应$h(t)$（离散形式为$h[n]$）表示。这里以离散形式为例，这个系统的物理实现是一个$N$阶抽头滤波器，滤波器系数可以写成一个列向量:

$${\mathbf{h}} = {[h(0),h(1),\ldots,h(N - 1)]^T}$$

使指标逼近最小值的方法有很多种，如`steepest descent`（最佳梯度法）逼近。也就是对$C$关于${\mathbf{h}}$求偏导，再令偏导为0那一套方法。
