...
$$
R_{world}=R_1R_2\cdots R_N
$$
$$
W_{world}=R_1S_1R_2S_2\cdots R_NS_N
$$
$$
S_{world}=R_{world}^{-1}W_{world}
$$
$$
s = \operatorname{diag}(S_{world})
$$

上式中，$1,\ldots,N$是根节点到叶子节点的编号，所有矩阵均采用列优先矩阵（因此使用左乘，例如$R_{world}$含义是先使用$R_N$进行变换到父空间，然后依次……直到使用$R_1$变换到世界空间）。$R_i$是只包含自身旋转信息的3x3旋转矩阵。$S_i$是只包含自身缩放信息的3x3对角矩阵。
...
M_{world}=M_1M_2=
\begin{pmatrix}
\bar{R}_1\bar{S}_1 & \bar{T}_1 \\
0 &  1
\end{pmatrix}
\begin{pmatrix}
\bar{R}_2\bar{S}_2 & \bar{T}_2 \\
0 &  1
\end{pmatrix}=
\begin{pmatrix}
\bar{R}_1\bar{S}_1\bar{R}_2\bar{S}_2 & \bar{R}_1\bar{S}_1\bar{T}_2+\bar{T}_1 \\
0 & 1
\end{pmatrix}
$$
...
T_{world}=
\begin{pmatrix}
 \bar{R}_1\bar{S}_1\bar{T}_2+\bar{T}_1
\end{pmatrix}
$$
...
$$
S_{world}=R_{world}^{-1}W_{world}
=(\bar{R}_1\bar{R}_2)^{-1}(\bar{R}_1\bar{S}_1\bar{R}_2\bar{S}_2)
=(\bar{R}_2^{-1}\bar{S}_1\bar{R}_2)\bar{S}_2
$$

一般来说，$S_{world}$是一个非对角矩阵，即主对角线之外也有非零值。从另外一个角度理解，将子空间的标准正交基变换到父空间后，这组基除了模长发生了变换，也不再两两垂直了——这也叫子节点发生了剪切（Shear）。当我们只用一个三维分量表示缩放时，势必要丢失这组基之间的夹角信息，这也是`LossyScale`名字的由来。只有当父节点是均匀缩放（$\bar{S}_1=s_1 I$）、或子节点没有相对旋转（$\bar{R}_2=I$）时，$S_{world}$才会退化为对角矩阵，`LossyScale`也才包含完整的缩放信息。
...
$$
L = \bar{R}_1\bar{S}_1\bar{R}_2\bar{S}_2
$$

以子节点本地的x轴基向量$e_x = (1,0,0)^T$为例，其经过$L$变换后，模长是：
$$
s_{world, x} = \lVert L e_x\rVert = \lVert\bar{R}_1\bar{S}_1\bar{R}_2\bar{S}_2 e_x\rVert
$$

最后一次旋转$\bar{R}_1$不改变模长，直接可得：
...
$$
S_{world}=(\bar{R}_2^{-1}\bar{S}_1\bar{R}_2)\bar{S}_2
$$

而似是而非的算法其实是：

$$
S_{world}=(\bar{R}_2s_1)\bar{S}_2
$$
